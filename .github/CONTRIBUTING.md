...
[Hiratake Web](https://hiratake.dev/) への貢献をご検討くださりありがとうございます🙏  
開発・編集に参加される場合は、本ドキュメントをお読みください👀

## 📩 Create an Issue

新機能の提案、不具合や誤字・脱字の報告は、このリポジトリの [Issues](https://github.com/Hiratake/hiratake-web/issues) に記載してください。
...
このリポジトリでは、 [Visual Studio Code](https://code.visualstudio.com/) の使用を推奨しています。  
推奨する拡張機能は `.vscode/extensions.json` に記載しておりますので、適宜インストールするなどの対応をおねがいいたします。

### 📦️ Package Manager

このリポジトリでは、パッケージマネージャーに [pnpm](https://pnpm.io/ja/) を採用しています。  
`npm` や `yarn` を使用している場合は、ご変更ください。

[mise](https://mise.jdx.dev/) を使用している場合は `mise install` を実行し、リポジトリで使用しているバージョンの `Node.js` と `pnpm` をインストールすることができます。
...
3. 必要な環境変数を設定します。`mise` を使用している場合は `mise.local.toml.example` ファイルを `mise.local.toml` としてコピーし、各環境変数を設定してください。
   - `NUXT_OG_IMAGE_SECRET` - `pnpm dlx nuxt-og-image generate-secret` で生成されたキー
4. 開発を開始します。

インストールされているパッケージは、GitHub 上で [Renovate](https://www.mend.io/renovate/) を用いて更新するか、`pnpm up --latest` コマンドを用いて更新してください。  
リリース直後のパッケージはインストールできず、5 日（7200 分）経過したバージョンのみインストール可能です。
